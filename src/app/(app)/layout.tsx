    ...
    <NotificationsProvider notifications={notifications} nombreNonLues={nombreNonLues}>
      ...
      <div className="flex w-full flex-1 flex-col overflow-x-clip lg:flex-row lg:overflow-x-visible">
        ...
        <div className="mx-auto flex w-full max-w-2xl flex-1 flex-col lg:mx-0 lg:max-w-none">
          <header className="relative flex items-center justify-between gap-2.5 bg-bg px-4 pb-3 pt-4 sm:px-8 lg:hidden print:hidden">
            ...
            <div className="flex shrink-0 items-center gap-2">
              ...
              {/* Cercles visibles de 36 px inchangés, cible tactile 44 px via
                  -m-1 + p-1 (même motif que NotificationsCloche juste à côté). */}
              <Link
                href="/inviter"
                aria-label="Mon équipe"
                className="-m-1 flex h-11 w-11 shrink-0 items-center justify-center rounded-full p-1 text-ink focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary"
              >
                <span className="flex h-9 w-9 items-center justify-center rounded-full bg-neutral-soft">
                  <svg
                    aria-hidden="true"
                    width="16"
                    height="16"
              ...
              {profilActuel && (
                ...
                >
                  <span
                    aria-hidden="true"
                    className={`flex h-9 w-9 items-center justify-center rounded-full border-2 border-surface text-[12px] font-bold shadow-[0_0_0_1.5px_var(--color-border)] ${couleurProfilActuel.fond} ${couleurProfilActuel.texte}`}
                  >
