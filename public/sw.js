...
self.addEventListener('push', (event) => {
  ...
  const options = {
    ...
    icon: '/icon-192',
    badge: '/icon-badge',
    data: { url: donnees.url || '/', notificationId: donnees.notificationId || null },
  }
...
self.addEventListener('notificationclick', (event) => {
  ...
  // Marque la ligne `notifications` comme lue avant de résoudre le
  // focus/navigate : un fetch brut (pas de server action, inutilisable
  // depuis un service worker) vers une Route Handler dédiée, RLS bornant
  // déjà l'update à l'utilisateur courant côté serveur. Erreur réseau
  // avalée : ne doit jamais bloquer l'ouverture/le focus de l'app.
  const marquerLue = notificationId
    ? fetch('/api/notifications/marquer-lue', {
        method: 'POST',
