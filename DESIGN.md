...
- **Hover/Focus :** anneau de focus visible (`focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary`) — généralisé à la navigation, l'accueil, les éléments globaux (Lot 1), au périmètre communication/organisation (Lot 2), au périmètre suivi officinal (Lot 3), au périmètre référentiels/catalogues : Carnet, Fournisseurs, Huiles essentielles, Chaussures, Documents (Lot 4), et aux comptes/réglages : écrans publics, Profil, sélecteurs d'officine/d'identité, Inviter, Diagnostics (Lot 5). Retour tactile global au tap (`active:scale-95`, `prefers-reduced-motion` respecté).
...
- **Erreur :** deux motifs cohabitent selon le contexte. Pour une erreur d'action ponctuelle sans notion de champ précis, le toast global (`role="alert"`, voir Toasts) ou un texte `text-[12px] text-rec` sous/à côté du contrôle (ex. `champ-audio.tsx`, `notifications-parametres.tsx`) suffisent, inchangés depuis le Lot 2. Pour un **formulaire d'authentification** (connexion, inscription, bienvenue — premier cas réel de la règle) établi au Lot 5 : `aria-invalid={true}` sur le ou les champs concernés par l'erreur serveur, `aria-describedby` les reliant au message d'erreur, ce message en `role="alert"` (annoncé immédiatement, pas seulement visible), et une bordure `border-rec` (au lieu de `border-border`) sur les champs en cause tant que l'erreur reste affichée — la seule variation du token existant, pas une couleur nouvelle. Une erreur d'authentification reste générique (« Email ou mot de passe incorrect », pas de distinction par champ) : les deux champs concernés (email + mot de passe) portent alors `aria-invalid`/la bordure d'erreur ensemble, faute de pouvoir désigner lequel est fautif. À reprendre tel quel pour tout futur formulaire avec une erreur serveur associable à un ou plusieurs champs.
...
**Note technique (`useFermerAvecRetour`, corrigée au Lot 5) :** pour une sheet qui ne monte qu'à son ouverture (JSX conditionnel, ex. `ModaleConfirmation` appelée depuis un formulaire), React Strict Mode (développement uniquement) rejouait deux fois les effets du même montage et pouvait refermer le panneau juste après l'avoir ouvert — un `history.back()` de nettoyage asynchrone entrait en course avec le `history.pushState` du second appel. Corrigé en différant ce `history.back()` (`setTimeout(…, 0)`, annulable) et en réutilisant l'entrée d'historique fictive existante si l'effet est relancé avant son échéance, plutôt que d'en empiler une seconde. Sans effet en production (Strict Mode ne s'y applique pas) ; voir `src/lib/use-fermer-avec-retour.ts` pour le détail complet et le rapport du Lot 5 pour les propriétés démontrées au banc. Ne réimplémentez pas ce hook localement — toute nouvelle sheet conditionnelle en hérite automatiquement.

### Modale de confirmation (composant signature)
Remplace `window.confirm()` partout dans l'app : sheet remontant du bas sur mobile (centrée à partir de `sm:`), rendue via `createPortal(..., document.body)` pour échapper à tout ancêtre `transform`. Piège à focus complet (Tab/Shift+Tab bouclent dans la boîte), Échap, retour du focus à l'élément déclencheur à la fermeture, verrouillage du scroll de la page tant qu'elle est ouverte (`document.body.style.overflow = 'hidden'`). Deux variantes : Annuler/Confirmer, ou une liste de choix (ex. « cette occurrence »/« toute la série »). Focus initial toujours sur le bouton Annuler (y compris en variante à choix, où il n'est pourtant pas le premier élément du DOM) — c'est pourquoi ce composant garde son propre piège à focus plutôt que le hook partagé `usePiegeFocus` (qui ne sait viser que « le premier élément focusable » ; voir son commentaire de tête et le rapport du Lot 5).

**Confirmation avant une remise à zéro (établi au Lot 5) :** tout bouton « Réinitialiser » qui efface un formulaire entier sans repli (plan de posologie, calculateur/posologie d'huiles essentielles) passe par `ModaleConfirmation` avant d'agir — titre explicite (« Effacer toutes les lignes saisies ? »), bouton de confirmation en variante destructive. Exception : si le formulaire est déjà vide, la réinitialisation reste immédiate, sans modale (rien à perdre, une confirmation y serait un frein inutile). Ne s'applique qu'à une remise à zéro qui perd une saisie réelle — un bouton qui réinitialise un simple filtre de recherche/affichage (ex. `fil-de-messages.tsx`) n'a rien à perdre et n'a pas besoin de ce motif.
...
### Écrans publics (connexion, inscription, bienvenue)
Seule concession au mode `Operate` du reste de l'app : une carte centrée (`max-w-sm`, `rounded-3xl`, `border border-border`, `shadow-sm`) sur fond `bg-bg` plein écran, sans sidebar ni bottom nav (hors du groupe `(app)`). Repère de marque discret — libellé `Officio` en `font-mono uppercase tracking-[0.12em] text-primary-light`, au plancher de 12 px — suivi d'un titre `font-heading text-2xl`. Établi au Lot 5, cohérent avec un rapprochement du mode `Brand` sans nouveau token : première impression soignée, mais aucun effet qui ne serait pas au service de la tâche (créer un compte, se connecter). Champs à 16 px avec attributs de saisie mobile complets (`autoComplete` précis — `email`, `current-password`/`new-password`, `name` —, `inputMode`, `enterKeyHint`, `autoCapitalize="none"` et `spellCheck={false}` pour l'email). Boutons de soumission en pleine largeur, ≥ 44 px, jamais couverts par le clavier virtuel (carte centrée verticalement, pas de contenu superflu sous le bouton). Un lien secondaire (« Pas encore de compte ? En créer un ») reste une simple inclusion de texte, sans cible 44 px dédiée — inline dans une phrase, il relève de l'exception WCAG pour les liens intégrés à du texte courant, à condition de rester `focus-visible`. Pages d'erreur et 404 (racine et `(app)`) : même ton sobre, illustration ronde `bg-*-soft`, actions « Réessayer »/« Retour à l'accueil » à 44 px.
