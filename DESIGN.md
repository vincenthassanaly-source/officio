...
- **Hover/Focus :** anneau de focus visible (`focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary`) — généralisé à la navigation, l'accueil, les éléments globaux (Lot 1), au périmètre communication/organisation (Lot 2), et au périmètre suivi officinal : Ruptures de stock, Produits à recommander, Suivi CNO, Régularisations (liste + calendrier), Vaccins, Plan de posologie (Lot 3). Reste à faire dans les modules des Lots 4 et 5. Retour tactile global au tap (`active:scale-95`, `prefers-reduced-motion` respecté).
